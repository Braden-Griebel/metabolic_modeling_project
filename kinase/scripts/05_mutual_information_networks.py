"""
Generate mutual information networks for each of the kinase IMAT models,
and compute their centrality measures.
"""
...
PROCESSES = 12
N_NEIGHBORS = 5
ZERO_DELTA = 1e-30  # Cutoff for values to be considered 0
...
logging.basicConfig(
    filename=BASE_PATH / "diagnostics" / "05_mutual_information_networks.log",
    filemode="w",
    level=logging.INFO,
)

logger.info("Starting to find MI networks...")
# Start by calculating the MI adjacency matrices for all of the imat models
for sample_path in FLUX_SAMPLES_PATH.glob("*.parquet"):
    ...
    # Find the indices of all 0 columns
    full_index = flux_samples.columns
    max_min_diff = flux_samples.max(axis=0) - flux_samples.min(axis=0)

    # Scale the flux samples to be 0-1, trying to remove any influence of scale of the flux
    ...
    # Now, filter the scaled samples for only the columns that are not all 0
    ...
    # Further remove any columns where this resulted in a NaN or Inf
    scaled_samples = cast(pd.DataFrame, scaled_samples)
    # Calculate the MI for every rxn pair
    logger.info("Finding the mutual information network")
    mi_arr = (
    ...
    np.fill_diagonal(
        mi_arr, 0.0
    )  # Set the diaganal to 0  (this might not be neccesary, but not sure)

    # Create a sparse array to hold the full adjacency matrix
    ...
    full_mi_arr.data[full_mi_arr.data < 0] = 0.0
...
# Determine if a results dataframe has already been created, if it has read it in
...
for network_adj_path in ADJACENCY_MATRIX_PATH.glob("*"):
    ...
    mean_degree = (adj_arr.sum(axis=0) / 2) / len(
        REACTION_LIST
    )  # Divide by 2 due to duplication
    ...
    # Calculate the PageRank centratlity
    ...
    # Note, that these treat weight as distance so the
    # reciprocal is taken for the non-zero entries in the
    # adjacency matrix
    ...
    logger.info("Calculating betweeness centrality")
    betweeness_cent = pd.Series(
    ...
    # Save the values to the dataframe
    res_df.loc[betweeness_cent.index, "betweeness"] = betweeness_cent
